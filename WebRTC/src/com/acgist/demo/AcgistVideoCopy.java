...
@ServerEndpoint("/acgistdemo111.video/{uid}")
public class AcgistVideoCopy {
	// 最大通话数量
	...
	public void onMessage(String message, Session session, @PathParam("uid")String uid) {
		try {
			if(uid != null && user_user.get(uid) != null && AcgistVideoCopy.sessions.get(user_user.get(uid)) != null) {
	...
	public static boolean addUser(String uid, String oid) {
		if(oid != null && !oid.isEmpty()) {
			AcgistVideoCopy.user_user.put(uid, oid);
			AcgistVideoCopy.user_user.put(oid, uid);
			
			return false;
		} else {//如果是会议创建者，即oid链接中不传入，进入此方法
			AcgistVideoCopy.user_user.put(uid, null);
